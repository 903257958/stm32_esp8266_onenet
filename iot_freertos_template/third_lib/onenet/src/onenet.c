...
#define PROID			"19B5g0iQ1N"

#define ACCESS_KEY		"0RPFy6HFX2Fvgwlpn1ExOzl7LafXv63LzQP972PHnQ8="
...
uint16_t OneNet_FillBuf(char *buf)
{
    ...
    memset(text, 0, sizeof(text));
	sprintf(text, "\"temperature\":{\"value\":%d},", dht11.temperature);
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"humidity\":{\"value\":%d},", dht11.humidity);
	strcat(buf, text);
    
    memset(text, 0, sizeof(text));
	sprintf(text, "\"led\":{\"value\":%s}", led.get_status(&led) ? "true" : "false");
	strcat(buf, text);
...
//				char *data_ptr = NULL;
				...
				if(led_json != NULL)
                {
					if(led_json->type == cJSON_True) led.on(&led);
					else led.off(&led);
				}
