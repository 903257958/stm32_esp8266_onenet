...
int oled_init(OLEDDev_t *pDev)
{
	...
	pPrivData->oled.config.scl_port = pDev->config.scl_port;
	pPrivData->oled.config.scl_pin = pDev->config.scl_pin;
	pPrivData->oled.config.sda_port = pDev->config.sda_port;
	pPrivData->oled.config.sda_pin = pDev->config.sda_pin;
	
	pPrivData->index = gIndex++;
	
	/* 配置I2C */
	i2c_init(&pPrivData->oled);
...
static int __oled_write_command(OLEDDev_t *pDev, uint8_t command)
{
	...
	pPrivData->oled.start(&pPrivData->oled);
	pPrivData->oled.send_byte(&pPrivData->oled, 0x78);		// 发送OLED的I2C从机地址
	pPrivData->oled.recv_ack(&pPrivData->oled);
	pPrivData->oled.send_byte(&pPrivData->oled, 0x00);		// 控制字节，给0x00，表示即将写命令
	pPrivData->oled.recv_ack(&pPrivData->oled);
	pPrivData->oled.send_byte(&pPrivData->oled, command);	// 写入指定的命令
	pPrivData->oled.recv_ack(&pPrivData->oled);
	pPrivData->oled.stop(&pPrivData->oled);
...
static int __oled_write_data(OLEDDev_t *pDev, uint8_t *data, uint8_t count)
{
	...
	pPrivData->oled.start(&pPrivData->oled);			//I2C起始
	pPrivData->oled.send_byte(&pPrivData->oled, 0x78);	//发送OLED的I2C从机地址
	pPrivData->oled.recv_ack(&pPrivData->oled);
	pPrivData->oled.send_byte(&pPrivData->oled, 0x40);	//控制字节，给0x40，表示即将写数量
	pPrivData->oled.recv_ack(&pPrivData->oled);
			
	/*循环count次，进行连续的数据写入*/
	for (i = 0; i < count; i ++)
	{
		pPrivData->oled.send_byte(&pPrivData->oled, data[i]);	//依次发送data的每一个数据
		pPrivData->oled.recv_ack(&pPrivData->oled);
	}
	pPrivData->oled.stop(&pPrivData->oled);				//I2C终止
